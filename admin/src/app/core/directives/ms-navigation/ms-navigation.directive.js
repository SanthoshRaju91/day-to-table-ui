(function() {
  ...
  /** @ngInject */
  function MsNavigationController($scope, msNavigationService, AuthService) {
    var vm = this;

    // Data
    // small hack for showing only the authentic routes for logged in user.
    if ($scope.root) {
      var navigation = msNavigationService.getNavigation($scope.root);
      var navigationChildren = navigation[0].children;
      navigation[0].children = [];
      for (var i = 0; i < navigationChildren.length; i++) {
        if (AuthService.authenticatedRoutes().includes(navigationChildren[i].state)) {
          navigation[0].children.push(navigationChildren[i]);
        }
      }
