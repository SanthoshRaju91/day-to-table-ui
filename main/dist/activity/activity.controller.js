...
(function () {
    ...
    ActivityController.$inject = ["RestService", "$state", "$stateParams", "$location", "$log", "$scope"];
    angular.module('app.activity', []).controller('ActivityController', ActivityController);
    ...
    function ActivityController(RestService, $state, $stateParams, $location, $log, $scope) {
         ...
         */
        RestService.get('getActivityById/' + activityId).then(function (response) {
        ...
        vm.bookActivity = function () {
            if (vm.adultCount == 0 && vm.childrenCount == 0) {
                ...
                RestService.post('/addBookingActivityFromUser', payload).then(function (response) {
